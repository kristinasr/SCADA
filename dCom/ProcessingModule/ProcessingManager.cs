...
namespace ProcessingModule
{
    ...
    public class ProcessingManager : IProcessingManager
    {
        ...
        private void ExecuteDigitalCommand(IConfigItem configItem, ushort transactionId, byte remoteUnitAddress, ushort pointAddress, int value)
        {
            ...
            List<PointIdentifier> pointList = new List<PointIdentifier> { v1, p1, p2, p3 };
            List<IPoint> points = storage.GetPoints(pointList);

            if (pointAddress == v1.Address && value == 1 && (points[1].RawValue == 1 || points[2].RawValue == 1 || points[3].RawValue == 1))
                return;
            if ((pointAddress == p1.Address || pointAddress == p2.Address || pointAddress == p3.Address) && value == 1 && points[0].RawValue == 1)
                return;
